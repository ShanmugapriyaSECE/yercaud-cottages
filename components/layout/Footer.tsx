export default function Footer() {
  return (
    <footer id="contact" className="bg-stone-950 text-white">
      <div className="mx-auto max-w-7xl px-6 py-20 lg:px-8">
        <div className="grid gap-12 md:grid-cols-2 lg:grid-cols-4">

          {/* Brand */}
          <div className="lg:col-span-1">
            <div className="flex flex-col leading-none">
              <span className="text-xl font-bold tracking-widest uppercase text-white">Sahana</span>
              <span className="text-[10px] font-medium tracking-[0.4em] text-amber-400 uppercase">Holidays</span>
            </div>
            <p className="mt-5 max-w-xs text-sm leading-6 text-white/50">
              Comfortable group stay and accommodation in the beautiful hills of Yercaud, Tamil Nadu.
            </p>
            <div className="mt-6 flex gap-3">
              <a
                href="https://wa.me/919150507580"
                target="_blank"
                rel="noopener noreferrer"
                className="flex h-9 w-9 items-center justify-center rounded-full border border-white/10 text-white/60 transition hover:border-amber-500 hover:text-amber-400"
              >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                </svg>
              </a>
              <a
                href="tel:+919150507580"
                className="flex h-9 w-9 items-center justify-center rounded-full border border-white/10 text-sm text-white/60 transition hover:border-amber-500 hover:text-amber-400"
              >
                📞
              </a>
            </div>
          </div>

          {/* Navigation */}
          <div>
            <h3 className="text-xs font-bold uppercase tracking-wider text-white/40">Navigate</h3>
            <div className="mt-5 flex flex-col gap-3 text-sm text-white/60">
              {[
                ["Home", "/"],
                ["About", "#about"],
                ["Rooms & Cottages", "#rooms"],
                ["Group Stay", "#group-stays"],
                ["Food", "#food"],
                ["Activities", "#experiences"],
                ["Explore Yercaud", "#explore"],
                ["Gallery", "#gallery"],
              ].map(([label, href]) => (
                <a key={label} href={href} className="transition hover:text-amber-400">{label}</a>
              ))}
            </div>
          </div>

          {/* Accommodation */}
          <div>
            <h3 className="text-xs font-bold uppercase tracking-wider text-white/40">Accommodation</h3>
            <div className="mt-5 flex flex-col gap-3 text-sm text-white/60">
              {[
                ["Deluxe Room", "/rooms/deluxe-room"],
                ["Triple Bedded Room", "/rooms/triple-bedded-room"],
                ["Family Room", "/rooms/family-room"],
                ["Family Suite Room", "/rooms/family-suite-room"],
                ["One Bedroom Cottage", "/rooms/one-bedroom-cottage"],
                ["Two Bedroom Cottage", "/rooms/two-bedroom-cottage"],
              ].map(([label, href]) => (
                <a key={label} href={href} className="transition hover:text-amber-400">{label}</a>
              ))}
            </div>
          </div>

          {/* Contact */}
          <div>
            <h3 className="text-xs font-bold uppercase tracking-wider text-white/40">Contact</h3>
            <div className="mt-5 space-y-4 text-sm text-white/60">
              <p>📍 Yercaud, Tamil Nadu 636601</p>
              <a href="tel:+919150507580" className="block transition hover:text-amber-400">
                📞 +91 91505 07580
              </a>
              <a
                href="mailto:yercaudcottages@gmail.com?subject=Group%20Stay%20Enquiry%20-%20Sahana%20Holidays%20Yercaud"
                target="_blank"
                rel="noopener noreferrer"
                className="block transition hover:text-amber-400"
              >
                ✉️ yercaudcottages@gmail.com
              </a>
              <div className="mt-4 flex flex-col gap-2">
                <a
                  href="tel:+919150507580"
                  className="inline-flex items-center justify-center rounded-full bg-amber-500 px-5 py-2.5 text-xs font-bold uppercase tracking-wider text-stone-950 transition hover:bg-amber-400"
                >
                  📞 Call Now
                </a>
                <a
                  href="https://wa.me/919150507580"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center justify-center gap-2 rounded-full border border-white/20 px-5 py-2.5 text-xs font-semibold text-white transition hover:bg-white hover:text-stone-900"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                  </svg>
                  WhatsApp Us
                </a>
              </div>
            </div>
          </div>
        </div>

        <div className="mt-16 flex flex-col items-center justify-between gap-4 border-t border-white/10 pt-8 text-xs text-white/30 sm:flex-row">
          <p>© 2026 Sahana Holidays, Yercaud. All rights reserved.</p>
          <p>Group Stay &amp; Accommodation in Yercaud, Tamil Nadu</p>
        </div>
      </div>
    </footer>
  );
}
